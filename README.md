...
and command line tools, but it does not fully replace the authored Ultimate
Talkie patch data yet.

It builds ScummVM-compatible Talkie Editions from assets that you already own.
No game data, Ultimate Talkie patch files, or builder-derived assets are shipped
in this repository. The project automates extraction, patch application, audio
processing, resource inspection, and resource packing with native command line
tools and a Python CLI.
...
SCUMMKit still requires a small set of authored data files from the original
Ultimate Talkie builder folders:

- MI1: `tools/patch10.000`, `tools/patch10.001`, and `tools/monster.tbl`.
- MI2: `tools/patch02.000`, `tools/patch02.001`, and `tools/monster.tbl`.

Those files are not generated by the shipped builder scripts. The patch files
encode authored changes to classic SCUMM resources, and `monster.tbl` maps the
patched game resources to speech archive entries. SCUMMKit can inspect and
classify these files, and it has native MI1 SBL data plus a speech manifest
prototype, but it does not yet have a complete native replacement for the
binary patches or the final MONSTER table IDs.

For that reason, `--builder` currently means "read the required local Ultimate
Talkie patch data from this folder." It does not mean SCUMMKit is running the
builder's Windows tools. The builder files remain local user-provided inputs and
must not be committed into this repository or redistributed as generated game
output.
...
| ---------------------------------- | ----------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The Secret of Monkey Island        | Ogg validated, builder data needed  | Native extraction, speech processing, music conversion, SBL generation/injection, and archive packing. Still requires local Ultimate Talkie `patch10.*` and `monster.tbl` files. FLAC/MP3/raw are not currently validated for the MI1 build pipeline. |
| Monkey Island 2: LeChuck's Revenge | Ogg validated, builder data needed  | Native extraction, speech processing, and archive packing. Still requires local Ultimate Talkie `patch02.*` and `monster.tbl` files. FLAC/MP3 use the same compressed archive path when encoders are available. Raw `monster.sou` generation is not implemented. |
...
`Monkey1.pak`, applies local Ultimate Talkie patch files with `bspatch`,
...
applies local Ultimate Talkie patch files, processes `Speech.xwb` and
...
- `monster.tbl`: table from the Ultimate Talkie builders mapping MONSTER/SOU
  ...
  samples belong in the ScummVM archive. Native generation is under
  investigation but does not replace the builder table today.
...
- `patch10.000` / `patch10.001`: MI1 Ultimate Talkie binary patches for
  classic SCUMM resource files. These remain local builder inputs.
...
- The build still requires local Ultimate Talkie patch data from the builder
  folders: `patch10.*`/`monster.tbl` for MI1 and `patch02.*`/`monster.tbl` for
  MI2. SCUMMKit does not redistribute or fully regenerate those files.
...
- The original Ultimate Talkie Edition builder folder for the game you want to
  build, used only as a local source for the required patch/table data.

SCUMMKit only automates the local build process. The required builder patch
data remains user-provided and is not included in this repository. The generated
output folder contains LucasArts/Disney game assets derived from your
installation and must not be redistributed.

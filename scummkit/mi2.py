...
def build(options: BuildOptions) -> None:
    ...
    require_file(patch_data / "patch02.000", "MI2 patch file")
    require_file(patch_data / "patch02.001", "MI2 patch file")
    ...
    if options.dry_run:
        ...
        runner.log("")
        runner.log(format_dependency_report("mi2", options.builder))
        runner.clean_dir(out)
        return

    try:
        monster.validate_table_for_game(patch_data / "monster.tbl", "mi2")
    ...
    runner.run(["bspatch", src000, out / "monkey2.000", patch_data / "patch02.000"])
    runner.run(["bspatch", src001, out / "monkey2.001", patch_data / "patch02.001"])
    ...
    build_note = write_build_note(game="mi2", out=out, pak=pak, patch_data_source=patch_data_source.root, audio=options.audio)

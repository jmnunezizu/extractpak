...
def generate_sbl_chunks(commands, samples_wav, work, sample_rate, verbose):
    ...
    for command in commands:
        ...
        chunk = sbl.wav_to_sbl_bytes(temp_wav)
...
def inject_mi1_sbl(builder, samples_wav, monkey000, monkey001, work, sample_rate=22050, verbose=False, commands=None):
    ...
    shutil.copy2(monkey000, pre_sbl / "monkey.000")
    shutil.copy2(monkey001, pre_sbl / "monkey.001")

    before_000 = sha256(monkey000)
    before_001 = sha256(monkey001)
    with tempfile.TemporaryDirectory(prefix="mi1-sbl-", dir=work) as temp_dir:
        sbl_chunks = generate_sbl_chunks(commands, samples_wav, Path(temp_dir), sample_rate, verbose)

    index_data = xor_data(monkey000.read_bytes())
    resource_data = xor_data(monkey001.read_bytes())
    new_index, new_resource_data, injected = rebuild_resource_data(
        resource_data, index_data, commands, sbl_chunks, verbose
    )
    ...
    monkey000.write_bytes(xor_data(new_index))
    monkey001.write_bytes(xor_data(new_resource_data))
    after_000 = sha256(monkey000)
    after_001 = sha256(monkey001)
...
def main():
    ...
    try:
        ...
        if before_001 == after_001:
            raise InjectError("monkey.001 did not change after SBL injection")
        return 0
    except (OSError, subprocess.CalledProcessError, sbl.SblError, InjectError) as error:
